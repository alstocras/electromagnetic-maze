...
## An enum made specifically for use with the [method get_debug_info] function.[br]
## Note that the values are specifically started
## [b]after[/b] the [constant Performance.MONITOR_MAX] value
## to allow for values for this enum to be used along side values from the
## [enum Performance.Monitor] enum without further ambiguity.[br]
## Also note that values in this enum start [b]after[/b]
## [constant Performance.MONITOR_MAX] and not [b]at[/b]
...
## (a choice sometimes made with other builtin enums).
enum DebugInfoTypes{
...
static func quick_editor_file_dialog(when_confirmed := Callable(),
	...
	file_select.access = access
	file_select.dialog_hide_on_ok = true
...
## returning [param unsupported_default] when the current platform does not support
## [method DirAccess.is_link],
...
## [param compression_level] is the specific compression level to use when compressing.[br]
## Since -1 is a valid ZIP compression level
## (correlating to the internal library's definition of a default compression level to use,
## [b]not[/b] to be confused with the default level set in the [ProjectSettings],
## of which can also include this -1 default value),
## the value used by this function to indicates the compression level should default
## to the value in [ProjectSettings] is -2 or lower.[br]
## Note that specifying a individual compression level is only supported in Godot v4.5 and up,
## so this parameter will raise an error if anything besides the [ProjectSettings]
## default value is specified to be used
## (including both by specifying the actual value in [ProjectSettings]
## or by specifying this to -2 or lower, as noted above).
static func compress_zip_async(source_path:String,
	...
	var proj_comp_level = ProjectSettings.get_setting("compression/formats/zlib/compression_level")
	if comp_level <= -2:
		comp_level = proj_comp_level
	var supports:bool = not (Engine.get_version_info()["major"] <= 4 and
	...
	if supports:
		packer.compression_level = comp_level
	...
	if err == OK:
		...
		# we should still try to close,
		# but we should maintain the earliest error to return
		packer.close()
...
static func generate_version_py(to_path:String) -> int:
	...
	if err == OK and not ver_file.store_line('module_config=""'):
		err = ver_file.get_error()
	var line := 'docs="%s"#Autogenerated"' % ["latest" if is_latest else "stable"]
	if err == OK and not ver_file.store_line(line):
		err = ver_file.get_error()
...
## [br]
## The array in [param successfully_copied_buffer] will have
## all the paths that were created without errors during copying appended to it.[br]
## [br]
...
static func copy_recursive(from_path:String,
	...
	for dir in DirAccess.get_directories_at(from_path):
		...
		if (from_dir != to_dir and not from_path in ignore_folders):
			var err := copy_recursive(from_dir,
										to_dir,
										ignore_folders,
										max_files,
										delete_on_fail,
										successfully_copied_buffer
										)
			if err != OK:
...
static func delete_recursive(path:String,
								successfully_removed_buffer := PackedStringArray()
								) -> int:
	var err:int = FAILED
...
## falling back to deleting recursively
## (as moving a directory to the recycling is inherently recursive)
## it in case that fails for whatever reason
...
## all files successfully created from copying will be removed first.
## Otherwise, an error occurring during a copy will result in any
## successfully coped files to remain where they were.
static func move_recursive(from:String,
	...
	var err := copy_recursive(from,
								to,
								ignore_folders,
								max_files,
								delete_on_fail,
								successfully_moved_buffer
								)
	if err == OK:
		err = delete_recursive(from)
		if err != OK:
			# still try to blindly cleanup everything else that was moved first,
			# lets hope that that error was only specific to a single part of the path
			# and doesn't apply to the other stuff moved
...
## it will then attempt to fall back to the project theme before falling back to
## the default theme.
...
## Check if the [enum Variant.Type] enum value [param type] is some from of "Packed Array".[br]
## Note that this check uses the string name of the variant type to determine
## if its a packed array or not,
## so this may be compatible with other extention that provide more "Packed Array" types.
static func typeof_is_packed_array(type:int) -> bool:
	var s := type_string(type)
	return s.begins_with("Packed") and s.ends_with("Array")

## Check if the [enum Variant.Type] enum value [param type] is some from "Array",
## either by being [constant Variant.TYPE_ARRAY] or by
## [method typeof_is_packed_array] returning true.
static func typeof_is_any_array(type:int) -> bool:
	return type == TYPE_ARRAY or typeof_is_packed_array(type)

## A method focused on taking in the many forms enums are represented in godot
## and returning a [Dictionary] that correlates enum's names to the enum's values.[br]
## [br]
## When using gdscript and referring to constant dictionaries
## or enums also defined in other non-extension scripts,
## it's possible to access them like any other constant property in a class
## (as it it literally a constant [Dictionary]),
## however, this is not consistent between versions or
## classes defined in the [ClassDB].[br]
## This method attempts to simplify that
## by allowing for all types of enums to be retrieved in their dictionary form
## simply by providing the class's name and the enum's name in that class.
## This method will also consistently return enums that can also be inherited
## from parent classes regardless of if the parent is a [Script] or a [ClassDB]
## defined class (or both, in the case that is ever possible).[br]
## While [param name_or_object] accepts many different way to refer to a class,
## the most consistent (and suggested) method is to supply the class's name
## (or the path to the script if the script is an unnamed class)
## to [param name_or_object] and the enum's name to [param enum_name].
## However, the following are also acceptable way to look up enums:[br]
## [br]
## - If [param name_or_object] is a [Dictionary]
## that dictionary will be returned verbatim
## (while still [param enforce_values_of_int], if set).
## This is the only situation where [param enum_name] is entirely unused,
## and therefore ignored.[br]
## - If [param name_or_object] is a [Object] that's not a [Script],
## it will reference that object's class and potentially attached [Script]
## if possible for enums instead.[br]
## - If [param name_or_object] is a [Script],
## that script's will be used for getting finding the enum.[br]
## [br]
## Setting [param enforce_values_of_int] will assert that the
## returned enum must have values of the [int] type.
## While typically this isn't a concern,
## since [Script]'s enums are only defined as constant dictionaries,
## making it technically possible for typos of names to include dictionaries
## with non-[int] values.
## While [param enforce_values_of_int] defaults to [code]true[/code],
## it can be disabled for a menial speed boost when calling this method.
## This methods is indented for use with methods like
## make_int_enum_hint_string] or other means handling exported enum values
## in the inspector.
static func enum_extract_dict(name_or_object:Variant,
								enum_name := "",
								enforce_values_of_int := true,
								enforce_keys_of_string_like := true
								) -> Dictionary:
	# how passing in a gdscript enum type should (ideally) happen
	if typeof(name_or_object) == TYPE_DICTIONARY:
		var typed_dicts:bool = (Engine.get_version_info().major >= 4 and
								Engine.get_version_info().minor >= 4 and
								name_or_object.is_typed()
								)

		var kt:int = TYPE_NIL
		if typed_dicts and name_or_object.is_typed_key():
		...
		if enforce_keys_of_string_like:
			if kt != TYPE_NIL: #If the keys are variant, we should check them manually
				...
				if string_key_count == key_types.size():
					...
					kt = TYPE_STRING_NAME
				# otherwise, just leave it variant,
				# it's allowable (though perhaps this should change later on)

		var vt:int = TYPE_NIL
		if typed_dicts and name_or_object.is_typed_value():
	...
	if typeof(name_or_object) in [TYPE_STRING, TYPE_STRING_NAME, TYPE_OBJECT]:
		assert(enum_name != "")

		var ret := {}

		var cls_name:String = ""
		# don't use [get_class_name] here
		# because we want the [ClassDB] name specifically
		...
		# don't use [class_name_normalize]
		# or [get_class_name] here,
		# because we want the script specifically,
		# or else null
		if typeof(name_or_object) in [TYPE_STRING, TYPE_STRING_NAME]:
		...
		if script != null:
			...
			if script.get_base_script() != null:
				ret.merge(enum_extract_dict(script.get_base_script(),
											enum_name,
											enforce_values_of_int)
							)
...
## Takes a list of enums
## (in the form of a dictionary with [String] keys,
## another array (packed or otherwise) of string enum names,
## or string enum names directly),
## and attempts to merge them all into a single hint_string.
## Intended for use in property lists with the hint of [constant PROPERTY_HINT_ENUM_SUGGESTION].
## This will [method assert] that all enum's the names
## (ignoring the values all together, if even provided)
## has no common names with any other.[br]
## For converting an enum into it's [Dictionary] form consistently,
## see [method enum_extract_dict].[br]
## Note that this is intended specifically for use with [constant PROPERTY_HINT_ENUM_SUGGESTION].
## For [constant PROPERTY_HINT_ENUM],
## use [method make_int_enum_hint_string].[br]
...
## or string enum names directly),
## and attempts to merge them all into a single hint_string.
## Intended for use in property lists with the hint of [constant PROPERTY_HINT_ENUM].
## This will [method assert] that all enum's the values (but not names)
## has no common values with any other.[br]
## For converting an enum into it's [Dictionary] form consistently,
## see [method enum_extract_dict].[br]
## Note that this is intended specifically for use with [constant PROPERTY_HINT_ENUM].
## For [constant PROPERTY_HINT_ENUM_SUGGESTION],
## use [method make_string_suggestion_enum_hint_string].[br]
...
## a integer value correlating to [enum DebugInfoTypes].
	...
	if typeof(name_or_id) in [TYPE_STRING_NAME, TYPE_STRING]:
		...
		match(name_or_id):
			...
			DebugInfoTypes.OS_DISTRIBUTION_NAME_OR_NAME:
				if OS.get_distribution_name().is_empty():
					return OS.get_name()
				return OS.get_distribution_name()
			DebugInfoTypes.OS_NAME_WITH_DISTRIBUTION:
				if (OS.get_distribution_name().is_empty() or
					OS.get_distribution_name() == OS.get_name()
					):
					return OS.get_name()
				return "%s (%s)" % [OS.get_name(), OS.get_distribution_name()]
			...
			DebugInfoTypes.TEXT_SERVER_FEATURE_SIMPLE_LAYOUT:
				...
				return text_server.has_feature(TextServer.FEATURE_SIMPLE_LAYOUT)
			DebugInfoTypes.TEXT_SERVER_FEATURE_BIDI_LAYOUT:
				var text_server := TextServerManager.get_primary_interface()
				return text_server.has_feature(TextServer.FEATURE_BIDI_LAYOUT)
			DebugInfoTypes.TEXT_SERVER_FEATURE_VERTICAL_LAYOUT:
				var text_server := TextServerManager.get_primary_interface()
				return text_server.has_feature(TextServer.FEATURE_VERTICAL_LAYOUT)
			DebugInfoTypes.TEXT_SERVER_FEATURE_SHAPING:
				var text_server := TextServerManager.get_primary_interface()
				return text_server.has_feature(TextServer.FEATURE_SHAPING)
			DebugInfoTypes.TEXT_SERVER_FEATURE_KASHIDA_JUSTIFICATION:
				var text_server := TextServerManager.get_primary_interface()
				return text_server.has_feature(TextServer.FEATURE_KASHIDA_JUSTIFICATION)
			DebugInfoTypes.TEXT_SERVER_FEATURE_BREAK_ITERATORS:
				var text_server := TextServerManager.get_primary_interface()
				return text_server.has_feature(TextServer.FEATURE_BREAK_ITERATORS)
			DebugInfoTypes.TEXT_SERVER_FEATURE_FONT_BITMAP:
				var text_server := TextServerManager.get_primary_interface()
				return text_server.has_feature(TextServer.FEATURE_FONT_BITMAP)
			DebugInfoTypes.TEXT_SERVER_FEATURE_FONT_DYNAMIC:
				var text_server := TextServerManager.get_primary_interface()
				return text_server.has_feature(TextServer.FEATURE_FONT_DYNAMIC)
			DebugInfoTypes.TEXT_SERVER_FEATURE_FONT_MSDF:
				var text_server := TextServerManager.get_primary_interface()
				return text_server.has_feature(TextServer.FEATURE_FONT_MSDF)
			DebugInfoTypes.TEXT_SERVER_FEATURE_FONT_SYSTEM:
				var text_server := TextServerManager.get_primary_interface()
				return text_server.has_feature(TextServer.FEATURE_FONT_SYSTEM)
			DebugInfoTypes.TEXT_SERVER_FEATURE_FONT_VARIABLE:
				var text_server := TextServerManager.get_primary_interface()
				return text_server.has_feature(TextServer.FEATURE_FONT_VARIABLE)
			DebugInfoTypes.TEXT_SERVER_FEATURE_CONTEXT_SENSITIVE_CASE_CONVERSION:
				var text_server := TextServerManager.get_primary_interface()
				return text_server.has_feature(TextServer.FEATURE_CONTEXT_SENSITIVE_CASE_CONVERSION)
			DebugInfoTypes.TEXT_SERVER_FEATURE_USE_SUPPORT_DATA:
				var text_server := TextServerManager.get_primary_interface()
				return text_server.has_feature(TextServer.FEATURE_USE_SUPPORT_DATA)
			DebugInfoTypes.TEXT_SERVER_FEATURE_UNICODE_IDENTIFIERS:
				var text_server := TextServerManager.get_primary_interface()
				return text_server.has_feature(TextServer.FEATURE_UNICODE_IDENTIFIERS)
...
## Checks if any file are changed as according to the vcs,
## always returning [code]false[/code] if the vcs is not enabled.
...
static func focus_chain_right(controls:Array[Control],
								loop:= false,
								unique_paths_allowed := false
								) -> void:
	...
	for i in range(range_max):
		...
		controls[i].focus_neighbor_right = controls[i].get_path_to(controls[next_i],
																	unique_paths_allowed
																	)
...
static func focus_chain_left(controls:Array[Control],
								loop:= false,
								unique_paths_allowed := false
								) -> void:
	var range_max := controls.size()
	...
	for i in range(range_max):
		var next_i := wrapi(i+1, 0, controls.size())
		controls[i].focus_neighbor_left = controls[i].get_path_to(controls[next_i],
																	unique_paths_allowed
																	)
...
static func focus_chain_top(controls:Array[Control],
							loop:= false,
							unique_paths_allowed := false
							) -> void:
	var range_max := controls.size()
	...
	for i in range(range_max):
		var next_i := wrapi(i+1, 0, controls.size())
		controls[i].focus_neighbor_top = controls[i].get_path_to(controls[next_i],
																	unique_paths_allowed
																	)
...
static func focus_chain_bottom(controls:Array[Control],
								loop:= false,
								unique_paths_allowed := false
								) -> void:
	var range_max := controls.size()
	...
	for i in range(range_max):
		var next_i := wrapi(i+1, 0, controls.size())
		controls[i].focus_neighbor_bottom = controls[i].get_path_to(controls[next_i],
																	unique_paths_allowed
																	)
...
static func focus_chain_next(controls:Array[Control],
								loop:= false,
								unique_paths_allowed := false
								) -> void:
	var range_max := controls.size()
	...
	for i in range(range_max):
		var next_i := wrapi(i+1, 0, controls.size())
		controls[i].focus_next = controls[i].get_path_to(controls[next_i],
															unique_paths_allowed
															)
...
static func focus_chain_previous(controls:Array[Control],
									loop:= false,
									unique_paths_allowed := false
									) -> void:
	var range_max := controls.size()
...
## [member Control.focus_previous] will be cleared.[br]
## NOTE: This overrides any existing paths set for [member Control.focus_previous].
static func focus_chain_all(controls:Array[Control],
							loop := false,
							unique_paths_allowed := false
							) -> void:
